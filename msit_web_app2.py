...
def show_page():
    ...
    <div class="CarbonPageTitle">
        <strong>🚀 정부지원사업 통합 대시보드 (v2)</strong>
        <div class="func">
            <button class="ty1Button">설정</button>
        </div>
    </div>
    ...
    with c1:
        st.markdown('<div class="CarbonBox"><div class="CarbonBoxTitle"><b>🔍 검색 필터</b></div>', unsafe_allow_html=True)
        ...
        st.markdown('</div>', unsafe_allow_html=True) # End CarbonBox
    ...
    with c2:
        ...
        st.markdown(f"### 📋 공고 목록 ({len(filtered)}건)")
            ...
            for idx, item in enumerate(filtered):
                ...
                with col:
                    ...
                    <div class="CarbonBox" style="padding: 20px; min-height: 250px; margin-bottom: 20px;">
                        <span class="badge" style="background-color: #e3f2fd; color: #1565c0;">{item.get('deptName')}</span>
                        ...
                        <p style="font-size: 12px; color: #666; margin-bottom: 5px;">📅 {item.get('pressDt')}</p>
                        <p style="font-size: 12px; color: #666;">👤 {item.get('managerName', '-')}</p>
                    </div>
                    """, unsafe_allow_html=True)
